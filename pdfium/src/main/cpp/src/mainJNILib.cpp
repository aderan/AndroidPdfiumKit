...
JNI_FUNC(jlong, PdfiumCore, nativeTextLoadPage)(JNI_ARGS, jlong pagePtr) {
    FPDF_PAGE page = reinterpret_cast<FPDF_PAGE>(pagePtr);
...
JNI_FUNC(void, PdfiumCore, nativeTextClosePage)(JNI_ARGS, jlong textPagePtr) {
...
JNI_FUNC(jint, PdfiumCore, nativeTextCountRects)(JNI_ARGS, jlong textPagePtr, jint start, jint count) {
    FPDF_TEXTPAGE textPage = reinterpret_cast<FPDF_TEXTPAGE>(textPagePtr);
    return FPDFText_CountRects(textPage, start, count);
}

JNI_FUNC(jobject, PdfiumCore, nativeTextGetRect)(JNI_ARGS, jlong textPagePtr, jint rectIndex) {
    FPDF_TEXTPAGE textPage = reinterpret_cast<FPDF_TEXTPAGE>(textPagePtr);
    FS_RECTF rect;
    ...
    jclass clazz = env->FindClass("android/graphics/RectF");
    jmethodID constructorID = env->GetMethodID(clazz, "<init>", "(FFFF)V");
    return env->NewObject(clazz, constructorID, (float)left, (float)top, (float)right, (float)bottom);
}

JNI_FUNC(jint, PdfiumCore, nativeTextGetText)(JNI_ARGS,
                                              jlong textPagePtr,
                                              jint startIndex,
                                              jint count,
                                              jcharArray result) {
...
JNI_FUNC(jint, PdfiumCore, nativeTextGetBoundedText)(JNI_ARGS,
    ...
    int retCount = FPDFText_GetBoundedText(textPage, left, top, right, bottom, cResult, count);
    if (count > 0) {
